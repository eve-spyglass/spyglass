...
class Map(object):
    ...
    DOTLAN_BASIC_URL = "https://evemaps.dotlan.net/svg/{0}.svg"
    styles = Styles()
    ...
    def svg(self):
        ...
        if float(self.marker["opacity"]) > 0.0:
            delta = datetime.datetime.utcnow().timestamp() - float(
                self.marker["activated"]
            )
    ...
    def __init__(
        self, region, svgFile=None, setJumpMapsVisible=False, setSatisticsVisible=False
    ):
        self.region = region
        self.width = None
        self.height = None
        ...
        if (
            self.region == "Providencecatch"
            or self.region == "Providence-catch-compact"
        ):
        ...
        if not svg or svg.startswith("region not found"):
            try:
                ...
                cache.putIntoCache(
                    "map_" + self.region, svg, 24 * 60 * 60
                )  ###evegate.secondsTillDowntime() + ### 60 * 60)
                ...
                if not svg or svg.startswith("region not found"):
                    t = (
                        ...
                        "updates: https://github.com/Crypta-Eve/spyglass".format(
                            type(e), str(e)
                        )
                    )
    ...
    def _extractSystemsFromSoup(self, soup, scale):
        ...
        for use in soup.select("line"):
            use.attrs["x1"] = str(
                (float(use.attrs["x1"]) - svg_width / 2.0) * scale + svg_width / 2.0
            )
            use.attrs["y1"] = str(
                (float(use.attrs["y1"]) - svg_height / 2.0) * scale + svg_height / 2.0
            )
            use.attrs["x2"] = str(
                (float(use.attrs["x2"]) - svg_width / 2.0) * scale + svg_width / 2.0
            )
            use.attrs["y2"] = str(
                (float(use.attrs["y2"]) - svg_height / 2.0) * scale + svg_height / 2.0
            )
        ...
        for symbol in symbols:
            ...
            for element in symbol.select(".sys"):
                ...
                mapCoordinates["center_x"] = (
                    mapCoordinates["x"] + 1.0 + 56.0 / 2.0
                )  # (mapCoordinates["width"] / 2.0))
                mapCoordinates["center_y"] = mapCoordinates["y"] + (
                    mapCoordinates["height"] / 2.0
                )
                try:
    ...
    def _prepareSvg(self, soup, systems):
        ...
        svg.attrs = {
            ...
            if key not in ["style", "onmousedown", "viewbox"]
        }
        ...
        for defs in svg.select("defs"):
            for tag in defs.select("a"):
                tag.attrs = {
                    ...
                    if key not in ["target", "xlink:href"]
                }
                tag.name = "a"
        ...
        for systemId, system in self.systemsById.items():
            ...
            text = "stats n/a"
            style = (
                "text-anchor:middle;font-size:7;font-weight:normal;font-family:Arial;"
            )
            system.svgtext = soup.new_tag(
                "text",
                x=coords["center_x"],
    ...
    def setJumpbridges(self, jumpbridgesData):
        ...
        for bridge in jumpbridgesData:
            ...
            if False:
                ...
                line = soup.new_tag(
                    "path",
                    d="M{} {} Q {} {} {} {}".format(x1, y1, mx, my, x2, y2),
                    visibility="hidden",
                    fill="transparent",
...
class System(object):
        ...
        if charname in self.__locatedCharacters:
            ...
            if not self.__locatedCharacters:
                try:
                    ...
                    if elem is not None:
                        logging.debug(
                            "removeLocatedCharacter {0} Decompose {1}".format(
                                charname, str(elem)
                            )
                        )
                        elem.decompose()
                    ...
                    logging.critical(
                        "Error in removeLocatedCharacter  {0}".format(str(e))
                    )
                    pass
    ...
    def setStatus(self, newStatus, alarm_time=datetime.datetime.utcnow()):
        ...
        if newStatus not in (
            states.NOT_CHANGE,
            states.REQUEST,
        ):  # unknown not affect system status
    ...
    def setStatistics(self, statistics):
        if self.svgtext is not None:
            if statistics is None:
                ...
                self.svgtext.string = (
                    "j-{jumps} f-{factionkills} s-{shipkills} p-{podkills}".format(
                        **statistics
                    )
                )
    ...
    def update(self):
            ...
            for maxDiff, alarmColour, lineColour in self.ALARM_COLORS:
                if alarmTime < maxDiff:
                    if self.backgroundColor != alarmColour:
                        ...
                        for rect in self.svgElement("rect"):
                            if "location" not in rect.get(
                                "class", []
                            ) and "marked" not in rect.get("class", []):
                                rect["style"] = self.SYSTEM_STYLE.format(
        ...
        if self.status in (states.ALARM, states.CLEAR):
            ...
            self.secondLine.string = "{m:02d}:{s:02d}".format(
                m=minutes, s=seconds, ticker=self.ticker
            )
    ...
    def updateStyle(self):
        for i in range(5):
            self.ALARM_COLORS[i] = (
                self.ALARM_COLORS[i][0],
                self.styles.getCommons()["alarm_colours"][i],
